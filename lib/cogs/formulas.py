...
class Formulas(Cog):
	...
	async def mindSlots(self, ctx, freezingSpellLevel: int, baseLevel: int, totalInt: int):
		...
		embed.add_field(name='Earth Spike, Drain Life', value=f'{7 if (totalMindSlots//8) > 7 else totalMindSlots//8}', inline=True)
		embed.add_field(name='LoV, MS, SG', value=f'{7 if (totalMindSlots//10) > 7 else totalMindSlots//10}', inline=True)
		embed.add_field(name='CL, CR, ES', value=f'{7 if (totalMindSlots//12) > 7 else totalMindSlots//12}', inline=True)
		embed.add_field(name='Comet, Tetra Vortex', value=f'{7 if (totalMindSlots//22) > 7 else totalMindSlots//22}', inline=True)
		embed.set_author(name=ctx.author.display_name, icon_url=ctx.author.avatar_url)
		embed.set_footer(text='Brought to you by Warlock Gang', icon_url=ctx.guild.icon_url)
